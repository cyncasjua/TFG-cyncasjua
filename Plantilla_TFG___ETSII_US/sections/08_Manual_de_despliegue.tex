...
%--------------------------------------------------
\section{Programación externa de la purga RGPD}

La lógica de purga de mensajes privados huérfanos está implementada en el backend y se expone mediante el endpoint:

\begin{lstlisting}
POST https://sevillaneando-backend.onrender.com/scheduler/run-purga-datos
\end{lstlisting}

Para garantizar que esta tarea se ejecute de forma periódica aunque el proveedor de hosting suspenda el proceso del backend por inactividad, se configura un cron externo que invoque dicho endpoint una vez al día, en horario de baja actividad. La planificación recomendada es:

\begin{lstlisting}
0 4 * * *
\end{lstlisting}

Esta expresión ejecuta la purga diariamente a las 04:00. La frecuencia diaria es suficiente porque la política de retención elimina mensajes privados huérfanos únicamente cuando han superado los 30 días de antigüedad.

En un despliegue real, este endpoint debería protegerse mediante un secreto compartido enviado en cabecera o mediante autenticación administrativa. En el contexto académico del proyecto se documenta la necesidad de dicha protección como medida previa a una publicación abierta al público.
...
\begin{itemize}
    ...
    \item[$\square$] El cron externo diario de purga RGPD invoca \texttt{POST /scheduler/run-purga-datos}.
