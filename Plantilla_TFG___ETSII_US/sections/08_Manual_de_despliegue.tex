...
\begin{figure}[H]
    ...
    \node[svcbox, right=2cm of backend] (firebase) {Firebase\\Auth + Storage};
...
\begin{itemize}
    ...
    \item \textbf{Firebase}: servicio externo de Google que gestiona la autenticación de usuarios y el almacenamiento de imágenes de perfil. No requiere despliegue propio.

    \item \textbf{Cloudinary}: servicio externo para el almacenamiento y transformación de imágenes de eventos. No requiere despliegue propio.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Carga de imágenes}: subida de foto de perfil (verifica Firebase Storage).
