...
\begin{itemize}
    ...
    \item \textbf{Eventbrite / Ticketmaster / webs institucionales y de ocio de Sevilla}: fuentes externas de eventos consultadas automáticamente mediante scraping. Eventbrite se procesa extrayendo tarjetas de eventos y bloques JSON-LD del HTML; la API oficial de Ticketmaster se consume directamente; el resto de webs (portal municipal, guías de ocio, teatros y fundaciones) se procesan mediante el scraper basado en \textbf{Google Gemini AI}. El backend ejecuta diariamente un \textit{cron job} que invoca cada scraper, transforma los datos al modelo interno y los persiste en PostgreSQL asignándolos al usuario técnico \textit{scraper-bot}.
    \item \textbf{Google Gemini AI}: modelo de lenguaje utilizado por el \texttt{GeminiScraperService} para extraer y estructurar eventos a partir del contenido HTML de siete webs de ocio e institucionales de Sevilla. El backend envía el texto limpio de cada página al modelo \textbf{Gemini 2.5 Flash} mediante la API de Google Generative AI y obtiene los eventos en formato JSON estructurado.
...
\hline
\multirow{10}{*}{Eventos} & POST /events & Crear evento \\
 ...
 & GET /events/moderacion/list & Listar eventos pendientes de moderación \\
 & GET /events/moderacion/publicos & Listar eventos públicos para moderación \\
...
A lo largo del desarrollo de \textbf{Sevillaneando} se han aplicado de forma explícita o implícita varios patrones de diseño reconocidos. Muchos de ellos son inherentes a los frameworks y librerías elegidos; otros han sido aplicados de forma deliberada como decisiones de arquitectura. En esta sección se identifican y justifican los más relevantes.

\subsection{Patrón Módulo (NestJS)}

NestJS organiza el backend en módulos independientes (\textit{EventosModule}, \textit{UsuariosModule}, \textit{CategoriasModule}, etc.), cada uno encapsulando su propio controlador, servicio y entidades. Este patrón favorece la separación de responsabilidades, facilita el mantenimiento y permite extender el sistema añadiendo nuevos módulos sin modificar los existentes. Es el patrón estructural central del backend.

\subsection{Patrón Estrategia (\textit{IScraper})}

El sistema de scraping implementa el patrón Estrategia: cada fuente de eventos externos (\texttt{SevillaScraperService}, \texttt{TicketmasterScraperService}, \texttt{GeminiScraperService}) implementa la interfaz \texttt{IScraper}, que define el contrato común de obtención y normalización de eventos. El servicio orquestador (\texttt{ScrapingService}) trabaja con la abstracción, no con implementaciones concretas. Esto permite añadir nuevas fuentes de datos sin modificar la lógica de orquestación.

\subsection{Patrón Repositorio (TypeORM)}

El acceso a la base de datos se realiza a través de los repositorios de TypeORM, que abstraen las operaciones de persistencia y consulta SQL. Los servicios de NestJS no interactúan directamente con SQL, sino con los repositorios tipados de cada entidad. Esto desacopla la lógica de negocio de los detalles de la base de datos y facilita las pruebas unitarias al poder sustituir el repositorio por un doble de prueba.

\subsection{Patrón Guard (autenticación y autorización)}

NestJS implementa los guards como componentes que interceptan cada petición antes de que llegue al controlador. El \texttt{FirebaseAuthGuard} verifica el token JWT en todas las rutas protegidas; el \texttt{RolesGuard} comprueba que el usuario tiene el rol requerido para cada endpoint. Este patrón centraliza la lógica de autenticación y autorización, evitando duplicarla en cada controlador.

\subsection{Patrón Interceptor (Axios)}

En el frontend, el cliente Axios centralizado actúa como interceptor: inyecta automáticamente el token de Firebase en la cabecera \texttt{Authorization} de cada petición y aplica lógica de reintento ante errores de red. Este patrón evita repetir la gestión de autenticación en cada llamada al servicio y garantiza un comportamiento uniforme ante fallos de conectividad.

\subsection{Patrón Contexto / Proveedor (React)}

La gestión de estado global del frontend se implementa mediante el patrón Contexto de React (\texttt{AuthContext}, \texttt{ThemeContext}, \textbf{SocketContext}, \texttt{NotificacionesContext}). Cada contexto actúa como proveedor de un estado compartido que los componentes hijos pueden consumir sin necesidad de \textit{prop drilling}. La separación en cuatro contextos independientes evita rerenders innecesarios: un cambio en el tema no provoca la actualización de los componentes suscritos al contexto de notificaciones.

\subsection{Patrón Observador (Socket.IO)}

La comunicación en tiempo real del chat se apoya en el patrón Observador: el servidor emite eventos (\texttt{mensaje-evento}, \texttt{mensaje-privado}) y los clientes suscritos a la sala correspondiente los reciben automáticamente. Socket.IO gestiona internamente la lista de suscriptores y la entrega de eventos, desacoplando al emisor del receptor.

\subsection{Patrón Puntuación multifactorial (motor de recomendaciones)}

El motor de recomendaciones aplica una variante del patrón de puntuación (\textit{scoring}): cada evento candidato recibe una puntuación calculada como suma ponderada de factores independientes (afinidad por categoría, proximidad geográfica, proximidad temporal, historial de interacciones). Añadir o modificar un factor de puntuación solo requiere cambiar su peso o su función de cálculo sin alterar el resto del algoritmo.

\subsection{Patrón Estado (flujo de moderación)}

El ciclo de vida de un evento sigue el patrón Estado: un evento puede encontrarse en estado \texttt{Pendiente}, \texttt{Aprobado} o \texttt{Rechazado}, y las transiciones entre estados están controladas por reglas de negocio definidas en el servicio. No todas las transiciones son válidas (por ejemplo, no se puede aprobar un evento ya aprobado), lo que garantiza la coherencia del flujo de moderación.
...
% Fila 3: Scrapers (tres columnas)
\node[box] (s1) at (-4.2,-3.4) {SevillaScraper\\(cheerio + JSON-LD)};
\node[box] (s2) at ( 0.0,-3.4) {TicketmasterScraper\\(API oficial)};
\node[box] (s3) at ( 4.2,-3.4) {GeminiScraper\\(extracción con IA)};

% Fila 4: Fuentes externas (bajo cada scraper)
\node[ext] (evtb) at (-4.2,-5.0) {Eventbrite\\(HTML + JSON-LD)};
\node[ext] (tick) at ( 0.0,-5.0) {Ticketmaster API};
\node[ext] (gem)  at ( 4.2,-5.0) {7 webs + Gemini AI};
...
Cada fuente de eventos externos (Eventbrite, Ticketmaster, webs institucionales y de ocio de Sevilla procesadas con Gemini AI) está implementada como un scraper independiente que implementa la interfaz \texttt{IScraper}. Esto permite añadir nuevas fuentes de datos sin modificar la lógica de orquestación del servicio de scraping. La alternativa de implementar el scraping como código monolítico habría acoplado la lógica de cada fuente con el proceso de guardado en base de datos, dificultando el mantenimiento cuando alguna de las fuentes externas cambia su estructura.
...
El contenido de eventos de la aplicación proviene de dos grandes bloques: eventos creados por usuarios y eventos obtenidos automáticamente de fuentes externas. Para estas últimas se utilizan tres estrategias complementarias:
