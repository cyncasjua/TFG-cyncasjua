...
\begin{itemize}
    ...
    \item \textbf{Eventbrite}~\cite{eventbrite} \textbf{/ Ticketmaster}~\cite{ticketmaster} \textbf{/ webs institucionales y de ocio de Sevilla}: fuentes externas de eventos consultadas automáticamente mediante scraping. Eventbrite se procesa extrayendo tarjetas de eventos y bloques JSON-LD del HTML; la API oficial de Ticketmaster se consume directamente; el resto de webs (portal municipal, guías de ocio, teatros y fundaciones) se procesan mediante el scraper basado en \textbf{Google Gemini AI}. El backend ejecuta diariamente un \textit{cron job} que invoca cada scraper, transforma los datos al modelo interno y los persiste en PostgreSQL asignándolos al usuario técnico \textit{scraper-bot}.
