...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners=3pt, minimum width=2.8cm, minimum height=0.75cm, align=center, fill=white},
    ext/.style={draw, rounded corners=3pt, minimum width=2.8cm, minimum height=0.75cm, align=center, fill=gray!12},
    arr/.style={-{Stealth[length=5pt]}, thick},
    darr/.style={latex-latex, thick, dashed},
]

% -- Columna izquierda: móvil  (x = -4.2) --------------------------
\node[box] (screen) at (-4.2,  0.0) {Pantalla\\(React Native)};
\node[box] (ctx)    at (-4.2, -1.6) {Contextos\\(Auth/Socket/Tema)};
\node[box] (axios)  at (-4.2, -3.2) {Servicios\\(Axios)};
\node[box] (fbase)  at (-4.2, -4.8) {Firebase SDK\\(cliente)};

% -- Columna central: backend  (x = 0) -----------------------------
\node[font=\tiny\itshape] at (0, 0.7) {Socket.IO / REST};
\node[box] (guard) at (0,  0.0) {FirebaseAuthGuard};
\node[box] (ctrl)  at (0, -1.6) {Controller\\(NestJS)};
\node[box] (svc)   at (0, -3.2) {Service\\(NestJS)};
\node[box] (orm)   at (0, -4.8) {TypeORM};
\node[box] (db)    at (0, -6.4) {PostgreSQL\\+ PostGIS};

% -- Columna derecha: servicios externos  (x = 4.2) ----------------
\node[ext] (fbauth) at (4.2,  0.0) {Firebase\\Authentication};
\node[ext] (cloud)  at (4.2, -1.6) {Cloudinary};
\node[ext] (osm)    at (4.2, -3.2) {OpenStreetMap};
\node[ext] (scrap)  at (4.2, -4.8) {Ticketmaster\\Portal Sevilla};

% -- Flechas verticales backend -------------------------------------
\draw[arr] (guard) -- (ctrl);
\draw[arr] (ctrl)  -- (svc);
\draw[arr] (svc)   -- (orm);
\draw[arr] (orm)   -- (db);

% -- Móvil → backend (horizontales en la misma fila) ---------------
\draw[arr] (ctx)   -- node[above,font=\tiny] {WS token}   (guard);
\draw[arr] (axios) -- node[above,font=\tiny] {HTTP/REST}  (ctrl);

% -- Backend → servicios externos (horizontales en la misma fila) --
...
% Pasa por debajo de todos los nodos (y = -7.2) para no cruzar ninguna caja
\draw[darr] (fbase.south) -- ++(0,-0.5)
    -| node[pos=0.3,below,font=\tiny] {JWT} (fbauth.south);

% -- Pantalla → OpenStreetMap (tiles) ------------------------------
% Pasa por encima del área de nodos (y=1.3) para no cruzar ninguna caja
\draw[arr] (screen.north) -- ++(0,1.3)
    -| node[pos=0.3,above,font=\tiny] {tiles} (osm.north);

% -- Service → Ticketmaster/Sevilla (cron, punteada) ---------------
\draw[arr,dashed] (svc) -- node[above,font=\tiny,sloped] {cron} (scrap);
...
El sistema de scraping implementa el patrón Estrategia: cada fuente de eventos externos (\texttt{TicketmasterScraper}, \texttt{PortalSevillaScraper}) implementa la interfaz \texttt{IScraper}, que define el contrato común de obtención y normalización de eventos. El servicio orquestador (\texttt{ScrapingService}) trabaja con la abstracción, no con implementaciones concretas. Esto permite añadir nuevas fuentes de datos sin modificar la lógica de orquestación.
...
Adicionalmente, la persistencia permite implementar el indicador de mensajes no leídos (\texttt{leido}), que informa al usuario de mensajes pendientes sin tener que entrar en cada conversación. El uso de Socket.IO garantiza la entrega inmediata cuando los participantes están conectados; la base de datos garantiza que el mensaje esté disponible independientemente del estado de conexión. Ambos mecanismos son complementarios y necesarios.
...
\begin{tikzpicture}[
    every node/.style={font=\small},
    box/.style={draw, rounded corners=3pt, minimum width=3.4cm, minimum height=0.75cm, align=center, fill=white},
    ext/.style={draw, rounded corners=3pt, minimum width=3.0cm, minimum height=0.75cm, align=center, fill=gray!12},
    arr/.style={-{Stealth[length=5pt]}, thick},
]

% Flujo central (eje x=0)
\node[box] (cron) at (0, 0.0) {Cron Job (3:00 AM)\\@nestjs/schedule};
\node[box] (orch) at (0,-1.6) {ScrapingService\\(orquestador)};
\node[box] (norm) at (0,-5.6) {Normalización al\\modelo Event};
\node[box] (db)   at (0,-7.2) {PostgreSQL\\(usuario scraper-bot)};

% Fuentes externas (fila y=-1.6, en columnas laterales — misma altura que orch)
\node[ext] (tick) at (-3.2,-1.6) {Ticketmaster API};
\node[ext] (web)  at ( 3.2,-1.6) {Portal Municipal\\Sevilla};

% Scrapers (fila y=-3.6, bajo las fuentes externas)
\node[box] (s1) at (-3.2,-3.6) {TicketmasterScraper\\(API oficial)};
\node[box] (s2) at ( 3.2,-3.6) {PortalSevillaScraper\\(scraping HTML)};

% Flechas
\draw[arr] (cron) -- (orch);
% orch → scrapers: baja primero, luego va horizontal (evita cruzar tick/web)
\draw[arr] (orch.south) -- ++(0,-0.6) -| (s1.north);
\draw[arr] (orch.south) -- ++(0,-0.6) -| (s2.north);
\draw[arr] (tick) -- (s1);
\draw[arr] (web)  -- (s2);
\draw[arr] (s1)   |- (norm);
\draw[arr] (s2)   |- (norm);
...
\begin{itemize}
    ...
    \item El modelo unidireccional es más natural para la relación entre un organizador habitual y su audiencia: no implica reciprocidad ni requiere aceptación por parte del otro usuario.
\end{itemize}
