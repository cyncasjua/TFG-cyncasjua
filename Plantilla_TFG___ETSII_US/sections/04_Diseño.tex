...
Registra los cambios que un moderador realiza directamente sobre un evento pendiente antes de aprobarlo. Almacena los campos modificados y el estado final de la solicitud (\texttt{pendiente}, \texttt{aprobada} o \texttt{rechazada}).
...
\begin{itemize}
    ...
    \item Un evento puede tener múltiples reseñas (1:N), múltiples mensajes de chat grupal (1:N) y múltiples solicitudes de edición (1:N).
    ...
    \item Una solicitud de edición (\texttt{EventEditRequest}) referencia al evento sobre el que se proponen cambios (N:1) y al moderador que la realiza (N:1).
\end{itemize}
...
El moderador recibe en su panel los eventos en estado pendiente y puede editarlos, aprobarlos o rechazarlos directamente, sin necesidad de solicitar permiso al creador. Al aprobar o rechazar un evento, este desaparece del listado de moderación.
...
Perfil de usuario & Vista del perfil de otro usuario & Todos \\
Amigos & Seguidores y seguidos del usuario & Todos \\
...
Los eventos no se eliminan físicamente del sistema. Esta decisión responde a tres motivos concretos:

\begin{itemize}
    \item \textbf{Consistencia de rutas}: una ruta puede referenciar eventos ya finalizados como parte de un itinerario. Eliminar el evento invalidaría silenciosamente cualquier ruta que lo contenga.
    \item \textbf{Valor social acumulado}: las reseñas y el chat del evento siguen siendo útiles después de que este haya terminado. Otros usuarios pueden descubrir el evento, consultar las valoraciones de quienes asistieron e informarse de cara a futuras ediciones del mismo o de eventos de la misma compañía o organizador.
    \item \textbf{Recomendaciones}: el historial de asistencia y las interacciones con eventos pasados alimentan el motor de recomendaciones. Sin esos registros, el algoritmo perdería señales de comportamiento relevantes para sugerir contenido afín.
\end{itemize}
...
El sistema implementa un grafo de seguimiento unidireccional entre usuarios: una persona puede seguir a otra sin que esa otra deba aceptar ni corresponder. Esta funcionalidad refuerza el carácter social de la aplicación al permitir descubrir planes a través de otras personas:

\begin{itemize}
    \item Desde el perfil de un usuario seguido se pueden consultar los eventos a los que va a asistir, lo que facilita coordinarse o descubrir planes que de otro modo no habrían aparecido en el listado general.
...
\begin{itemize}
    ...
    \item \textbf{Mensajes privados}: el campo de emisor o receptor se establece a \texttt{NULL} (\textit{SET NULL}), de forma que la conversación permanece visible para el otro participante bajo la etiqueta \emph{Usuario eliminado}. Borrar el hilo completo perjudicaría al otro usuario, que perdería su historial de conversación.
