...
Representa a cada persona registrada en el sistema. Sus atributos principales son el identificador UUID, el UID de Firebase, el correo electrónico, el nombre, la foto de perfil, el identificador público de la imagen en Cloudinary (\texttt{fotoPerfilPublicId}), la ubicación geográfica (punto PostGIS en SRID 4326), el rol (\texttt{user}, \texttt{moderator} o \texttt{admin}) y el array de intereses por categoría (\texttt{intereses}). Adicionalmente, almacena la fecha de aceptación de la política de privacidad (\texttt{privacyAcceptedAt}) y preferencias de personalización como el orden de categorías (\texttt{categoryOrder}) y los radios de búsqueda configurados (\texttt{radiusOptions}).

\subsection{Evento (\texttt{Event})}

Representa un evento cultural o de ocio. Sus atributos incluyen título, descripción, dirección, ubicación geográfica (punto PostGIS en SRID 4326), fechas de inicio y fin, precio (fijo o en rango), imagen principal, hasta cinco imágenes adicionales, estado de moderación (\texttt{Pendiente}, \texttt{Aprobado} o \texttt{Rechazado}), visibilidad (\texttt{privado}), enlace de acceso (\texttt{linkAcceso}) para eventos privados, recurrencia (\texttt{diario}, \texttt{semanal}, \texttt{quincenal} o \texttt{mensual}), fecha final de recurrencia (\texttt{recurrenciaFin}) y el indicador \texttt{hasMultipleDatesAvailable} para eventos importados con varias sesiones o fechas a consultar.
...
También puede almacenar referencias auxiliares al evento relacionado (\texttt{targetEventId}) o al usuario origen (\texttt{sourceUserId}), lo que permite abrir directamente la pantalla correspondiente desde la notificación.
...
\subsection{Solicitud de edición de evento (\texttt{EventEditRequest})}

Registra las modificaciones propuestas sobre un evento cuando deben pasar por revisión. Almacena el evento original, el usuario solicitante, los campos editados (título, descripción, dirección, ubicación, fechas, precio, visibilidad, enlace e imágenes), la fecha de creación, el estado de la solicitud (\texttt{pendiente}, \texttt{aprobada} o \texttt{rechazada}) y un motivo de rechazo opcional. Esta entidad permite separar la información publicada de una propuesta de cambio pendiente de moderación.
...
\multirow{19}{=}{Eventos}
 & POST /events & U & Body: \texttt{CreateEventDto} (title, description, address, location, fechaInicio, fechaFin, precio, precioMin, precioMax, privado, categoriaId, imagen, imagenes, recurrencia, recurrenciaFin) & 201, 400, 401 \\
\cline{2-5}
 & GET /events & — & Query: \texttt{userId?}, \texttt{lat?}, \texttt{lng?}, \texttt{radiusKm?}, \texttt{limit?}, \texttt{offset?} & 200 \\
\cline{2-5}
...
\cline{2-5}
 & GET /events/\{id\}/attendees/me & U & Path: \texttt{id} & 200, 401 \\
\cline{2-5}
...
\cline{2-5}
 & GET /events/user/\{userId\} & — & Path: \texttt{userId} & 200 \\
\cline{2-5}
 & GET /events/attending/\{userId\} & — & Path: \texttt{userId} & 200 \\
\hline
...
\cline{2-5}
 & GET /users/admin/stats & A & — & 200, 401, 403 \\
\cline{2-5}
...
\cline{2-5}
 & GET /users/\{id\}/siguiendo & U & Path: \texttt{id} & 200, 401 \\
\cline{2-5}
 & GET /users/\{id\}/seguidos & U & Path: \texttt{id} & 200, 401 \\
\cline{2-5}
 & GET /users/\{id\}/seguidores & U & Path: \texttt{id} & 200, 401 \\
\cline{2-5}
 & POST /users/upload-profile-image/firebase & U & Form: \texttt{file} (binary) & 201, 400, 401 \\
\hline
...
\cline{2-5}
 & GET /rutas/\{id\}/mi-calificacion & U & Path: \texttt{id} & 200, 401 \\
\cline{2-5}
...
\cline{2-5}
 & POST /recomendaciones/events/\{id\}/visitar & U & Path: \texttt{eventId} & 201, 401 \\
\cline{2-5}
 & POST /recomendaciones/events/\{id\}/valorar & U & Path: \texttt{eventId}; Body: \texttt{RateEventDto} & 201, 400, 401 \\
\cline{2-5}
 & GET /recomendaciones/events/\{id\}/valorar/me & U & Path: \texttt{eventId} & 200, 401 \\
\hline
...
\multirow{5}{=}{Solic. edición}
 & POST /event-edit-requests/\{eventId\}/\{userId\} & — & Path: \texttt{eventId}, \texttt{userId}; Body: \texttt{EventEditRequestDto} & 201, 400 \\
\cline{2-5}
 & PATCH /event-edit-requests/\{requestId\}/approve & — & Path: \texttt{requestId} & 200, 404 \\
\cline{2-5}
 & PATCH /event-edit-requests/\{requestId\}/reject & — & Path: \texttt{requestId}; Body: \texttt{motivoRechazo?} & 200, 404 \\
\cline{2-5}
 & GET /event-edit-requests/pending/\{eventId\} & — & Path: \texttt{eventId} & 200 \\
\cline{2-5}
 & GET /event-edit-requests & — & — & 200 \\
\hline
...
\cline{2-5}
 & POST /scraping/delete-all-events & A & — (solo fuera de producción) & 200, 403 \\
\hline

\multirow{4}{=}{Scheduler}
 & GET /scheduler/health & — & — & 200 \\
\cline{2-5}
 & POST /scheduler/run-scraping & — & — & 200 \\
\cline{2-5}
 & POST /scheduler/run-notifications & — & — & 200 \\
\cline{2-5}
 & POST /scheduler/run-purga-datos & — & — & 200 \\
\hline
...
Amigos & Módulo social con búsqueda de usuarios, seguidores, seguidos y amistades mutuas & Todos \\
Conexiones & Listado de seguidores y seguidos de un usuario concreto & Todos \\
...
Ayuda & Preguntas frecuentes y guía de uso de la aplicación & Todos \\
Política de privacidad & Política de tratamiento de datos personales & Todos \\
Atribuciones legales & Licencias y atribuciones de recursos de terceros & Todos \\
\hline
\end{tabular}%
}
\end{table}
...
La planificación de las tareas periódicas (scraping diario, envío de notificaciones y purga RGPD de mensajes) se gestiona mediante un servicio de \textit{cron} externo (cron-job.org) que invoca los endpoints \texttt{POST /scheduler/run-scraping}, \texttt{POST /scheduler/run-notifications} y \texttt{POST /scheduler/run-purga-datos}, en lugar de usar el planificador interno de NestJS (\texttt{@nestjs/schedule}).
