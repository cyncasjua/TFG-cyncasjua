...
\begin{itemize}
    ...
    \item Ejecución de tareas programadas (cron jobs) para scraping, notificaciones y purga RGPD de mensajes privados huérfanos.
\end{itemize}
...
|   |   |   |-- scraping/       (scrapers Eventbrite, Ticketmaster, Gemini, VisitaSevilla)
|   |   |   |-- scheduler/      (cron jobs scraping/notificaciones/purga y health check)
...
|       |   |-- context/        (AuthContext, SocketContext, ThemeContext, NotificacionesContext)
...
\begin{itemize}
    ...
    \item \textbf{scheduler}: tareas programadas con \texttt{@nestjs/schedule}. Ejecuta scrapers periódicamente, envía recordatorios de eventos próximos y realiza la purga RGPD de mensajes privados de cuentas eliminadas.
\end{itemize}
