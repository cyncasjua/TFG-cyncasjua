...
\begin{figure}[H]
    ...
    \node[box] (client) {Cliente móvil\\React Native 0.81 + Expo 54};
...
El cliente ha sido desarrollado con \textbf{React Native 0.81} y \textbf{Expo SDK 54}, lo que permite crear una aplicación multiplataforma (iOS y Android) con una única base de código en TypeScript.
...
\begin{itemize}
    ...
    \item Web scraping de eventos externos (Ticketmaster, Gemini AI).
...
\begin{itemize}
    ...
    \item \textbf{Firebase Storage}: almacenamiento de imágenes de usuarios.
    \item \textbf{Cloudinary}: almacenamiento y transformación de imágenes de eventos.
...
Node.js & 20.x LTS \\
npm & 10.x \\
TypeScript & 5.3.x (backend) / 5.9.x (frontend) \\
NestJS & 11.x \\
TypeORM & 0.3.20 \\
Socket.IO & 4.8.3 \\
Expo SDK & 54.x \\
...
El sistema requiere variables de entorno para conectar con los servicios externos y la base de datos. Deben crearse los archivos \texttt{.env} correspondientes antes de arrancar el sistema.

\subsection{Backend (\texttt{apps/backend/.env})}
...
\subsection{Frontend (\texttt{apps/frontend/.env})}
...
El contenedor queda disponible en \texttt{localhost:5433}. Si es la primera vez que se levanta, TypeORM creará automáticamente las tablas al arrancar el backend (opción \texttt{synchronize: true} en modo desarrollo).
...
Expo levanta el servidor de desarrollo y muestra un código QR. La aplicación puede ejecutarse en:
\begin{itemize}
    \item Dispositivo físico: escaneando el QR con la aplicación \textbf{Expo Go}.
    \item Emulador Android: pulsando \texttt{a} en la terminal.
...
|   |   |   |-- scraping/       (scrapers Ticketmaster, Gemini)
...
\begin{itemize}
    ...
    \item \textbf{events}: módulo principal. Gestiona el ciclo de vida de eventos (creación, aprobación por moderadores, edición, eliminación), reseñas y solicitudes de edición.
    \item \textbf{users}: gestión de perfiles, cambio de contraseña, asignación de roles y lista de amigos.
    ...
    \item \textbf{notificaciones}: generación y entrega de notificaciones en tiempo real mediante WebSockets.
    \item \textbf{recomendaciones}: motor de recomendaciones basado en preferencias de categoría y valoraciones previas.
    \item \textbf{scraping}: scrapers para importar eventos de Ticketmaster y mediante Gemini AI. Implementa el patrón estrategia con la interfaz \texttt{ScraperInterface}.
...
\begin{itemize}
    \item \textbf{No conecta con el backend}: verificar que \texttt{EXPO\_PUBLIC\_API\_URL} apunta a la IP de la máquina de desarrollo (no \texttt{localhost}) cuando se ejecuta en dispositivo físico.
...
\begin{itemize}
    ...
    \item React Native Documentation: \texttt{https://reactnative.dev}
    \item Expo Documentation: \texttt{https://docs.expo.dev}
    \item TypeORM Documentation: \texttt{https://typeorm.io}
    \item PostGIS Documentation: \texttt{https://postgis.net/documentation}
    \item Firebase Admin SDK: \texttt{https://firebase.google.com/docs/admin/setup}
    \item Conventional Commits: \texttt{https://www.conventionalcommits.org}
\end{itemize}
