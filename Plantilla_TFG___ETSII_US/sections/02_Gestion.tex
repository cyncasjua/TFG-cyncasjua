...
\begin{itemize}
    ...
    \item Promover la interacción social entre usuarios mediante chat en eventos, mensajería privada y opiniones en las valoraciones.
    ...
    \item Incluir notificaciones para informar al usuario sobre eventos relevantes, recordatorios y estado de los eventos.
\end{itemize}
...
El trabajo se ha organizado en sprints con objetivos concretos, combinando en una única sesión la revisión del trabajo realizado y la planificación del siguiente sprint. Este enfoque ha permitido mantener una evolución iterativa del sistema, adaptando el alcance en función del tiempo disponible y de las prioridades del proyecto.
...
El seguimiento del tiempo dedicado al proyecto se ha realizado con la herramienta \textbf{Clockify}, lo que ha permitido monitorizar el esfuerzo real invertido en cada fase y compararlo con la estimación inicial.
...
Adicionalmente, se ha configurado un pipeline de integración y entrega continua (CI/CD) mediante \textbf{GitHub Actions}, que automatiza la ejecución del linter y las comprobaciones de formato en cada push, contribuyendo a mantener la calidad del código a lo largo del desarrollo.
...
\begin{itemize}
    ...
    \item \textbf{Fase 5 – Despliegue y documentación}: despliegue del sistema y elaboración de la memoria y manuales.
\end{itemize}

Durante el desarrollo se ha definido un último sprint centrado en la implementación de rutas de eventos, considerado como uno de los objetivos principales del sistema. Paralelamente, se ha iniciado la redacción de la memoria y la ejecución del plan de pruebas, tal y como se estableció en las reuniones de seguimiento.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{5.2} Gestión de calendarios.
    \end{itemize}

    \item \textbf{Fase 6 -- Despliegue y documentación}
    \begin{itemize}
        \item \textbf{6.1} Despliegue del backend.
        \item \textbf{6.2} Publicación de la app.
...
\begin{itemize}
    \item Uso de herramientas gratuitas o con plan educativo (GitHub, Expo, Firebase).
    \item Base de datos PostgreSQL con extensión PostGIS (entorno local o servicios gratuitos).
    \item Almacenamiento de imágenes mediante Cloudinary (plan gratuito).
    \item Despliegue del backend en Google Cloud Run.
\end{itemize}
