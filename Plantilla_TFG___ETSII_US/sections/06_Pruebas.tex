...
Se han implementado \textbf{17 pruebas unitarias} sobre \texttt{users.service.ts}, cubriendo la creación y actualización de usuarios, el sistema de seguimiento social y la gestión de roles:
...
Las pruebas de integración verifican que la comunicación entre los distintos componentes del sistema funciona correctamente de extremo a extremo. Dado que el sistema está formado por un frontend React Native, un backend NestJS y una base de datos PostgreSQL con PostGIS, se han identificado los siguientes escenarios de integración relevantes:

\begin{itemize}
    \item \textbf{Frontend -- API REST}: se verifica que las llamadas HTTP realizadas desde el frontend llegan correctamente al backend, los parámetros son procesados y la respuesta JSON es interpretada correctamente por la capa de presentación.

    \item \textbf{API REST -- Base de datos}: se comprueba que las operaciones de creación, consulta, actualización y eliminación sobre los repositorios TypeORM se traducen correctamente en operaciones SQL sobre PostgreSQL, incluyendo las consultas geoespaciales con PostGIS (\texttt{ST\_DWithin}, \texttt{ST\_MakePoint}, \texttt{ST\_Distance}).

    \item \textbf{Backend -- Firebase Auth}: se verifica que el guard de autenticación (\texttt{FirebaseGuard}) valida correctamente los tokens JWT emitidos por Firebase y que las peticiones sin token o con token inválido son rechazadas con código HTTP 401.

    \item \textbf{Backend -- WebSockets}: se comprueba que los mensajes enviados desde la aplicación a través de Socket.IO llegan correctamente al servidor y se distribuyen a los participantes de la sala correspondiente.

    \item \textbf{Backend -- Cloudinary}: se verifica que las imágenes subidas desde la aplicación se almacenan correctamente en Cloudinary y que la URL generada queda guardada en la base de datos.
\end{itemize}

Estos escenarios se han validado de forma funcional durante la ejecución de las pruebas de aceptación documentadas en la sección siguiente, ya que cada caso de prueba ejecutado sobre la aplicación real ejercita implícitamente el flujo completo entre capas.
...
Durante la fase de pruebas con usuarios piloto (véase el Anexo de Usuarios Piloto), se detectaron problemas de compatibilidad en dispositivos Android reales que no habían aparecido en el emulador ni en las pruebas sobre iOS. Los principales problemas identificados fueron:

\begin{itemize}
    \item \textbf{Mapa no visible}: el componente de mapa (OpenStreetMap mediante \texttt{react-native-maps}) no se renderizaba correctamente en algunos dispositivos Android, mostrando una pantalla en blanco. El problema estaba relacionado con la configuración del proveedor de tiles y la gestión del token de acceso en la compilación de Expo Go para Android.
...
\hline
CP-01 & HU-10 / RF-01 & Correo válido, contraseña válida y datos mínimos de registro & Crear una nueva cuenta e iniciar sesión con las credenciales registradas & El sistema autentica al usuario y muestra la pantalla principal sin errores. La sesión queda iniciada correctamente. \\
\hline
CP-02 & HU-10 / RF-01 & Correo o contraseña incorrectos & Intentar iniciar sesión con credenciales inválidas & El sistema deniega el acceso y muestra un mensaje de error, sin permitir entrar. \\
\hline
CP-03 & HU-10 / RF-02 & Nombre, intereses, ubicación e imagen de perfil modificados & Editar los datos del perfil desde el menú lateral & Los cambios se reflejan tras guardar y persisten al volver a abrir el perfil. \\
\hline
CP-04 & HU-10 / RF-01 & Correo electrónico asociado a la cuenta & Solicitar recuperación de contraseña & El sistema inicia el flujo de recuperación y acepta el correo indicado. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-05 & HU-1 / RF-03 & Aplicación autenticada con eventos en BD & Acceder a la página principal & Se muestra el listado con imagen, nombre, categoría y fecha, sin errores de carga. \\
\hline
CP-06 & HU-2 / RF-03 & Evento existente con datos completos & Seleccionar un evento del listado & Se abre la ficha detallada con descripción, ubicación, precio, asistentes, valoraciones y acciones disponibles. \\
\hline
CP-07 & HU-14 / RF-04 & Categoría, texto de búsqueda, radio y estado & Aplicar filtros desde el buscador avanzado & El listado muestra únicamente los eventos que cumplen los filtros seleccionados. \\
\hline
CP-08 & HU-15 / RF-08 & Evento público activo & Pulsar la acción de apuntarse en el detalle & El usuario queda como asistente y el cambio se refleja en el detalle y en el listado de apuntados. \\
\hline
CP-09 & HU-15 / RF-08 & Evento finalizado & Acceder al detalle desde el listado de finalizados & La información del evento es visible pero la acción de apuntarse aparece deshabilitada. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-10 & HU-6 / RF-05 & Perfil con ubicación y eventos geolocalizados & Acceder al mapa de eventos & El mapa se carga correctamente con los eventos posicionados según su ubicación. \\
\hline
CP-11 & HU-6 / RF-05 & Distintos radios configurables & Cambiar el radio de búsqueda en el mapa & Los eventos visibles se ajustan al radio seleccionado. \\
\hline
CP-12 & HU-6 / RF-05 & Evento con coordenadas válidas & Seleccionar un marcador en el mapa & Se muestra el resumen del evento y al pulsarlo se accede a su ficha completa. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-13 & HU-8 / RF-09 & Texto de mensaje e imagen opcional & Enviar mensaje en el chat de un evento & El mensaje aparece en el chat y queda visible para los participantes. \\
\hline
CP-14 & HU-8 / RF-09 & Dos cuentas autenticadas en el mismo evento & Iniciar conversación privada desde la lista de asistentes & El chat privado se abre correctamente y los mensajes son visibles para ambas cuentas. \\
\hline
CP-15 & HU-9 / RF-11 & Puntuación entre 1 y 5 y comentario opcional & Valorar un evento desde su ficha de detalle & La valoración se registra y se refleja en la puntuación media del evento. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-16 & HU-11 / RF-12 & Usuario con intereses configurados y actividad previa & Abrir la sección ``Para ti'' & El sistema muestra recomendaciones coherentes con los intereses y la actividad del usuario. \\
\hline
CP-17 & HU-11 / RF-13 & Preferencias del usuario y eventos con fecha & Abrir la sección de rutas recomendadas & Se genera al menos una ruta con una agrupación coherente de eventos. \\
\hline
CP-18 & HU-11 / RF-13 & Datos mínimos de una ruta nueva & Crear una ruta manual y consultarla & La ruta queda almacenada y es visible en el listado con su vista de detalle. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-19 & HU-3 / RF-06 & Datos válidos para evento privado & Crear un evento privado & El sistema genera el evento y muestra un enlace de acceso compartible. El evento no aparece en el listado público. \\
\hline
CP-20 & HU-15 / RF-21 & Enlace privado válido & Acceder con otra cuenta usando el enlace & El usuario accede al evento y pasa a sus eventos visibles. \\
\hline
CP-21 & HU-15 / RF-21 & Sin enlace válido & Intentar acceder al evento sin enlace & El sistema deniega el acceso y el evento no aparece en ningún listado. \\
\hline
CP-22 & HU-3 / RF-20 & Evento privado ya creado & Acceder a ``Eventos privados'' y ``Mis eventos'' & El evento aparece en la sección correcta y puede editarse según las restricciones definidas. \\
\hline
CP-23 & HU-3 / RF-20 & Evento privado editado a público & Editar y cambiar visibilidad a pública & El evento pasa a estado pendiente y entra en cola de moderación, sin hacerse visible de inmediato. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-24 & HU-12 / RF-17 & Cuenta con rol administrador & Acceder al panel de administración & Se muestra la gestión de usuarios y se pueden modificar roles o eliminar cuentas. \\
\hline
CP-25 & HU-12 / RF-16 & Cuenta con rol administrador & Crear, editar o eliminar una categoría & Los cambios se reflejan en el sistema y la categoría queda disponible en filtros y formularios. \\
\hline
CP-26 & HU-4 / RF-07 & Cuenta moderador y evento pendiente & Acceder al panel de moderación & Se muestra la lista de eventos pendientes y se puede aprobar, rechazar o editar cada uno. \\
\hline
CP-27 & HU-3 / RF-07 & Evento del CP-23 en moderación & Verificar visibilidad del evento durante moderación & El evento no aparece en el listado público hasta que el moderador lo aprueba. \\
\hline
CP-28 & HU-12 / RF-15 & Cuenta con rol usuario estándar & Intentar acceder al panel de administración o moderación & El sistema deniega el acceso y no muestra esas opciones en la interfaz. \\
\hline
\end{longtable}
...
\textbf{ID} & \textbf{HU / RF} & \textbf{Entrada} & \textbf{Acción} & \textbf{Validación esperada} \\
\hline
CP-29 & HU-5 / RF-14 & Evento moderado por un moderador & Abrir la pantalla de notificaciones tras la resolución & El sistema muestra una notificación con el resultado de la moderación (aprobado o rechazado). \\
\hline
CP-30 & HU-7 / RF-14 & Perfil con ubicación y eventos cercanos & Revisar las notificaciones con la app activa & El sistema muestra avisos de eventos próximos a la ubicación del usuario. \\
\hline
\end{longtable}

\section{Resultados de las pruebas de aceptación}

Los 30 casos de prueba de aceptación se han ejecutado sobre los dos entornos descritos (local con emulador Android y desplegado sobre iPhone 16 Pro con Expo Go). La siguiente tabla recoge los resultados obtenidos:
...
Todos los casos de prueba han sido superados satisfactoriamente. El sistema se comporta de acuerdo con los requisitos funcionales definidos en el análisis tanto en el entorno de desarrollo local como en el entorno desplegado en producción.

\section{Evidencias visuales}

% [GUÍA PARA COMPLETAR] Inserta aquí las capturas de pantalla más representativas de cada bloque funcional.
% Para insertar una captura usa:
%
%   \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.38\textwidth]{figures/pruebas/cp05_listado.png}
%     \caption{CP-05: listado principal de eventos}
%   \end{figure}
%
% Guarda las imágenes en Plantilla_TFG___ETSII_US/figures/pruebas/
%
% Capturas mínimas recomendadas:
%   - CP-01 o CP-03: pantalla de registro o edición de perfil
%   - CP-05 o CP-06: listado o detalle de evento
%   - CP-07: buscador con filtros aplicados
%   - CP-10 o CP-12: mapa con marcadores
%   - CP-13: chat de evento con mensajes
%   - CP-16 o CP-17: pantalla de recomendaciones o rutas
%   - CP-19 o CP-20: flujo de evento privado
%   - CP-26: panel de moderación
%   - CP-29: pantalla de notificaciones

Las siguientes capturas de pantalla ilustran el comportamiento del sistema en los flujos más representativos de cada bloque funcional, obtenidas durante la ejecución de las pruebas de aceptación sobre el entorno desplegado.

% [GUÍA PARA COMPLETAR] Inserta aquí las figuras con las capturas reales de la app.
