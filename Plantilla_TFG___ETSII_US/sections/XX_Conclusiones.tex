...
\begin{itemize}
    ...
    \item \textbf{Interacción social}: implementado completamente. El sistema incluye chat de evento (sala grupal por evento), mensajería privada entre usuarios, valoraciones con puntuación y comentario, y módulo de amistad basado en seguimiento entre usuarios.
...
Desde el punto de vista funcional, el sistema permite consultar eventos, filtrarlos por criterios relevantes, visualizarlos en un mapa, crear eventos públicos y privados, moderar contenido, participar en chats, enviar mensajes privados, buscar usuarios, consultar sus perfiles, seguir o dejar de seguir a otras personas, ver los eventos a los que asisten, valorar eventos, recibir notificaciones y obtener recomendaciones personalizadas. Además, se han incorporado rutas de eventos recomendadas y rutas creadas por los propios usuarios, ampliando el alcance de la aplicación más allá de un listado convencional de actividades.
...
La integración de múltiples servicios externos (Firebase, Cloudinary, OpenStreetMap, Eventbrite, Ticketmaster, Google Gemini AI) puso de manifiesto la importancia de diseñar capas de abstracción que aislen al sistema de las particularidades de cada proveedor. Los errores de límite de peticiones del scraping y la variabilidad en las respuestas de la API de Gemini revelaron la necesidad de implementar pausas entre peticiones y gestión robusta de errores desde el principio, en lugar de añadirlos como parche posterior.
...
Asumir simultáneamente los roles de \textit{Product Owner}, \textit{Scrum Master} y desarrolladora exigió una autodisciplina especial en la priorización del backlog. La técnica de tallas de camiseta para la estimación, combinada con el seguimiento real de horas mediante Clockify (Anexo~\ref{anexo:reporte_horas}), permitió detectar a tiempo los desvíos de esfuerzo y ajustar el alcance de forma razonada.
...
\begin{itemize}
    \item \textbf{Incorporación de CI/CD y ajustes de calidad} (+9--10 h): la configuración de GitHub Actions no estaba prevista en la estimación original. Se decidió incorporarla al detectar que la base de código había alcanzado una complejidad suficiente como para justificar la automatización del linter, las comprobaciones de formato, la guía de estilo y los ajustes derivados del despliegue. El coste adicional se compensó con la reducción de tiempo dedicado a revisiones manuales en sprints posteriores.

    \item \textbf{Mayor complejidad del scraping} (+15--20 h): la gestión de errores 429, la variabilidad en la estructura de las respuestas externas, la extracción de direcciones, el tratamiento de fechas y la necesidad de implementar reintentos superaron la estimación inicial. El esfuerzo total se situó en torno a 30 horas, lo que convirtió esta desviación en una de las principales causas de que las funcionalidades opcionales de la Fase 5 no pudieran completarse.
...
\begin{itemize}
    ...
    \item \textbf{Módulo avanzado de amigos}: actualmente el sistema dispone de un módulo de amistad basado en seguimiento, con búsqueda de usuarios, perfiles públicos, opción de seguir o dejar de seguir, mensajería privada y consulta de eventos a los que asisten otros usuarios. Como ampliación futura, se podría incorporar una gestión más explícita de contactos cercanos o amistades confirmadas, invitaciones directas a eventos y recomendaciones sociales basadas en intereses compartidos.
...
\begin{itemize}
    ...
    \item \textbf{Claude (Anthropic)}: modelo de lenguaje utilizado para resolver dudas técnicas concretas (p.ej., uso de funciones espaciales de PostGIS, configuración de guardias de autenticación en NestJS), proponer mejoras visuales en la interfaz y como guía en procesos técnicos más elaborados, como el despliegue en producción y la revisión de vulnerabilidades de seguridad. Asimismo, la redacción de la memoria ha sido revisada con ayuda de esta herramienta para mejorar la expresión y coherencia del texto.
\end{itemize}
