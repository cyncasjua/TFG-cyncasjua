...
\begin{itemize}
    ...
    \item \textbf{Política de retención de datos}: el sistema no define actualmente un período máximo de retención para los datos personales ni dispone de un proceso automatizado de purga periódica. En particular, los mensajes privados de un usuario eliminado quedan huérfanos en la base de datos (el campo \texttt{autorId} se pone a \texttt{NULL} por la política \texttt{SET NULL}), pero el contenido del mensaje no se elimina. Una política de retención debería especificar que los mensajes privados se eliminan en cascada al eliminar la cuenta, o que su contenido se anonimiza tras un período definido.
...
\begin{itemize}
    \item \textbf{Cumplimiento RGPD}: las medidas básicas de consentimiento (checkbox en el registro con \texttt{privacyAcceptedAt}) y de derecho al olvido para imágenes de perfil (eliminación en Cloudinary al borrar la cuenta) ya están implementadas. Las tareas pendientes antes de un despliegue real son: implementar una pantalla de consentimiento informado específica para la geolocalización, definir y automatizar una política de retención de datos (incluyendo la eliminación o anonimización de mensajes privados al eliminar una cuenta) y establecer un procedimiento formal y documentado para el ejercicio del derecho al olvido. Estas limitaciones se identifican en la sección de consideraciones de seguridad y privacidad.
