...
\subsection{Compatibilidad multiplataforma}

El desarrollo en React Native con Expo promete una única base de código para iOS y Android, pero en la práctica existen diferencias de comportamiento que solo se manifiestan en dispositivos reales de cada plataforma. El hecho de haber desarrollado y probado íntegramente sobre iOS provocó que los problemas de compatibilidad con Android (renderizado del mapa, posicionamiento de elementos de interfaz) no fueran detectados hasta la fase de pruebas con usuarios piloto. La lección es clara: en proyectos multiplataforma es imprescindible incorporar dispositivos o emuladores de ambas plataformas en el ciclo de pruebas desde los primeros sprints, no solo al final.
...
\begin{itemize}
    ...
    \item \textbf{Corrección de compatibilidad con Android} (+8 h): durante las pruebas con usuarios piloto se detectaron problemas en dispositivos Android reales (mapa no visible, descuadres de pantalla) que no habían aparecido en el desarrollo sobre iOS ni en el emulador. La corrección requirió una iteración adicional en la fase de cierre, no contemplada en la planificación. Esta desviación pone de manifiesto la importancia de realizar pruebas sobre ambas plataformas desde fases tempranas del desarrollo.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Módulo de amigos}: los usuarios piloto destacaron la ausencia de una sección específica para gestionar contactos cercanos dentro de la aplicación. La idea es complementar el sistema de seguidores ya existente con una capa de ``amigos'' con reciprocidad, que permita ver los eventos planeados por las personas de confianza y recibir sugerencias de planes basadas en los gustos de la red social propia. Esta funcionalidad haría la plataforma más parecida a una red social de ocio y aumentaría la retención de usuarios.
