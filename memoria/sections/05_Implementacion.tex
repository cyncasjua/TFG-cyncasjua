...
\begin{itemize}
    \item \textbf{Descubrimiento}: \texttt{HomeScreen}, \texttt{EventDetailScreen}, \texttt{EventDetailLinkScreen} y \texttt{EventsMapScreen}. La pantalla principal concentra el listado, filtros, eventos recomendados, rutas sugeridas, menú lateral y acceso a eventos privados.
    ...
    \item \textbf{Administración e información}: \texttt{AdminScreen}, \texttt{CategoriesScreen}, \texttt{HelpScreen}, \texttt{PrivacyPolicyScreen} y \texttt{LegalAttributionsScreen}.
\end{itemize}
...
El backend incorpora un módulo de tareas programadas (\texttt{SchedulerModule}) basado en \textbf{@nestjs/schedule}. Este módulo ejecuta de forma autónoma dos tipos de tareas y expone una tercera tarea para ser invocada desde un disparador externo:
...
\begin{itemize}
    ...
    \item \textbf{Purga de datos RGPD}: implementada en \texttt{PurgaDatosScheduler} y expuesta mediante \texttt{POST /scheduler/run-purga-datos}. El proceso elimina los mensajes privados huérfanos —aquellos en los que tanto el emisor como el receptor tienen \texttt{NULL} porque ambas cuentas han sido eliminadas— con más de 30 días de antigüedad. En el despliegue se programa un cron externo diario, en horario de baja actividad, para invocar este endpoint y garantizar que la purga se ejecute aunque el proveedor suspenda el proceso del backend por inactividad.
\end{itemize}

El módulo también expone un controlador HTTP que permite lanzar las tres tareas manualmente durante el desarrollo o desde un sistema de orquestación externo, incluyendo los endpoints \texttt{POST /scheduler/run-scraping}, \texttt{POST /scheduler/run-notifications} y \texttt{POST /scheduler/run-purga-datos}. Para la purga RGPD se utiliza una planificación diaria con la expresión cron \texttt{0 4 * * *}, equivalente a una ejecución cada día a las 04:00.
