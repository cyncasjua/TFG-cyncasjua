...
\begin{itemize}
    ...
    \item Web scraping de eventos externos (Eventbrite Sevilla, Ticketmaster, visitasevilla.es y webs procesadas con Gemini AI).
    \item Ejecución de tareas programadas (cron jobs) para scraping y notificaciones.
\end{itemize}

El backend se organiza en módulos funcionales e infraestructurales independientes: \texttt{app}, \texttt{auth}, \texttt{events}, \texttt{users}, \texttt{rutas}, \texttt{chat}, \texttt{notificaciones}, \texttt{recomendaciones}, \texttt{categorias}, \texttt{scraping}, \texttt{scheduler}, \texttt{database} y \texttt{cloudinary}. El endpoint de comprobación de disponibilidad se expone desde el controlador de \texttt{scheduler}.
...
\begin{itemize}
    ...
    \item \textbf{Eventbrite}: importación de eventos de Sevilla mediante extracción HTML y JSON-LD.
    \item \textbf{visitasevilla.es}: importación de eventos locales mediante extracción HTML.
    \item \textbf{Google Gemini AI}: extracción inteligente de eventos desde webs externas mediante IA.
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{0.25\textwidth}|>{\raggedright\arraybackslash}p{0.22\textwidth}|>{\raggedright\arraybackslash}p{0.40\textwidth}|}
\hline
...
Docker & 4.x & \href{https://www.docker.com/products/docker-desktop}{docker.com/products/\allowbreak docker-desktop} \\
Firebase Admin SDK & 12.7.0 & \href{https://firebase.google.com/docs/admin/setup}{firebase.google.com/\allowbreak docs/admin/setup} \\
...
\begin{lstlisting}[basicstyle=\ttfamily\small, breaklines=true, breakatwhitespace=false, columns=flexible]
sevillaneando/
|-- apps/
|   |-- backend/
|   |   |-- src/
...
|   |   |   |-- scraping/       (Eventbrite, Ticketmaster, Gemini, VisitaSevilla)
|   |   |   |-- scheduler/      (cron jobs scraping/notificaciones y health)
...
|   `-- frontend/
|       |-- src/
|       |   |-- screens/        (31 pantallas de la aplicacion)
|       |   |-- components/     (componentes reutilizables)
|       |   |-- navigation/     (navegacion + deep linking)
|       |   |-- context/        (Auth, Socket, Theme, Notificaciones)
...
\begin{itemize}
    \item \textbf{auth}: integración con Firebase Admin SDK para validar tokens JWT. Incluye \texttt{FirebaseAuthGuard} y \texttt{RolesGuard}, aplicados en los controladores y endpoints protegidos.
    ...
    \item \textbf{scraping}: scrapers para importar eventos de Eventbrite Sevilla, Ticketmaster, visitasevilla.es y webs procesadas mediante Gemini AI. Implementa el patrón estrategia con la interfaz \texttt{ScraperInterface}.
...
\begin{itemize}
    ...
    \item \textbf{Eventos}: \texttt{EventsMapScreen}, \texttt{EventDetailScreen}, \texttt{EventDetailLinkScreen}, \texttt{CreateEventScreen}, \texttt{UserEditEventScreen}, \texttt{UserEventsScreen}, \texttt{ModeratorEventsScreen}, \texttt{ModeratorEditEventScreen}, \texttt{CalendarEventsScreen}, \texttt{SavedAndPrivateEventsScreen}, \texttt{AccessPrivateEventScreen}.
    \item \textbf{Rutas}: \texttt{RoutesListScreen}, \texttt{RouteDetailScreen}, \texttt{RoutePreviewScreen}, \texttt{CreateRouteScreen}.
    \item \textbf{Social}: \texttt{FriendsScreen}, \texttt{MessagesScreen}, \texttt{DirectMessageScreen}, \texttt{NotificacionesScreen}, \texttt{ProfileConnectionsScreen}.
    \item \textbf{Perfil}: \texttt{UserProfileScreen}, \texttt{EditProfileScreen}, \texttt{AdminScreen}.
    \item \textbf{Información}: \texttt{HelpScreen}, \texttt{PrivacyPolicyScreen}, \texttt{LegalAttributionsScreen}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Swagger / OpenAPI}: documentación interactiva de la API REST, accesible en \texttt{/api/docs} mediante \texttt{@nestjs/swagger}.
