...
En total se han implementado \textbf{44 pruebas unitarias automatizadas} sobre los tres servicios de mayor complejidad lógica del backend. La ejecución con \texttt{npm run test} produce el siguiente resultado:
...
\hline
\textbf{Total} & \textbf{44} & \textbf{44/44 superadas} \\
\hline
\end{tabular}
...
Las \textbf{44 pruebas unitarias} implementadas sobre los servicios de mayor complejidad lógica (\texttt{EventsService}, \texttt{RecomendacionesService} y \texttt{UsersService}) han superado correctamente todos los casos, incluyendo tanto flujos normales como situaciones de error. En particular, las 8 pruebas añadidas sobre el algoritmo de scoring de \texttt{RecomendacionesService} verifican aspectos críticos como la influencia de los intereses del usuario en la puntuación, el filtrado por radio geográfico, el bonus por historial de guardado y la normalización de acentos en categorías. Los \textbf{30 casos de prueba de aceptación} ejecutados sobre la aplicación real han confirmado que el sistema funciona correctamente en los dos entornos probados, tanto en Android (emulador local) como en iOS (iPhone 16 Pro con Expo Go mediante túnel y backend desplegado en Render).
