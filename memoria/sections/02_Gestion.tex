...
\begin{itemize}
    \item \textbf{OBJ-01.} Diseñar e implementar un sistema que permita listar eventos con información detallada (descripción, ubicación, imágenes, precio, etc.) y filtrado de eventos.

    \item \textbf{OBJ-02.} Permitir a los usuarios crear nuevos eventos, diferenciando entre eventos públicos y privados, e incorporando un sistema de moderación para garantizar la fiabilidad de los eventos públicos.

    \item \textbf{OBJ-03.} Permitir a los usuarios editar los eventos creados por sí mismos.

    \item \textbf{OBJ-04.} Integrar funcionalidades de geolocalización para mostrar eventos cercanos y permitir su visualización en un mapa interactivo para un acceso más cómodo.

    \item \textbf{OBJ-05.} Promover la interacción social entre usuarios mediante chat en eventos, mensajería privada, módulo de amistad basado en seguimiento entre usuarios y opiniones en las valoraciones.

    \item \textbf{OBJ-06.} Implementar un sistema de recomendaciones personalizado basado en la actividad del usuario (guardar un evento, apuntarse a un evento, compartir un evento, ver un evento, valorar un evento, etc.), sus intereses y su ubicación.

    \item \textbf{OBJ-07.} Desarrollar un sistema de rutas recomendadas de eventos que permita agrupar actividades según gustos y criterios de proximidad y horario.

    \item \textbf{OBJ-08.} Permitir a los usuarios crear rutas de eventos que sean visibles para el resto y que puedan valorarlas.

    \item \textbf{OBJ-09.} Incluir notificaciones para informar al usuario sobre eventos cercanos, recordatorios, seguidores nuevos y aprobación o rechazo de los eventos.
\end{itemize}
...
\begin{longtable}{p{3.5cm}p{1.5cm}p{1.5cm}p{4.5cm}p{3.5cm}}
...
La integración de Firebase, Cloudinary y Ticketmaster resultó más costosa de lo previsto, aunque manejable dentro del margen de cada sprint. \\
\midrule
...
Pérdida de datos o código por error en el sistema de control de versiones &
Baja & Alto &
Uso de Git con repositorio remoto en GitHub; commits frecuentes y política de ramas definida. &
No ocurrió. \\
...
\hline
Sistema de rutas & 12--14 h & $\sim$18--19 h & +5--7 h & Impacto de fechas múltiples del scraping \\
\hline
Compatibilidad Android & 0 h (no previsto) & $\sim$7--8 h & +7--8 h & Desarrollo solo sobre iOS hasta la fase de pruebas \\
\hline
CI/CD y calidad de código & 0 h (no previsto) & $\sim$9--10 h & +9--10 h & Decisión durante el Sprint 6 de automatizar checks \\
\hline
Funcionalidades opcionales (Fase 5) & 10 h & $\sim$3--4 h & $-$6--7 h & Reasignadas al desarrollo core \\
\hline
\end{tabular}%
}
\end{table}

En conjunto, las desviaciones al alza superan a las bajistas, lo que supone que el esfuerzo real del proyecto fue sensiblemente superior a las 300 horas presupuestadas. Esta diferencia no se considera un fracaso de planificación, sino un resultado habitual en proyectos de desarrollo de software donde el alcance técnico real solo se revela durante la implementación.
...
Fase 5 -- Ampliaciones y extras & 10 h & 284 € \\
Fase 6 -- Despliegue, documentación y cierre & 80 h & 2.268 € \\
\midrule
\textbf{Total} & \textbf{300 h} & \textbf{8.511 €} \\
\end{longtable}

Los importes de la tabla anterior se calculan aplicando el coste horario base de 28,37 €/h y redondeando al euro más cercano. La Fase 2 incluye la elaboración de mockups, modelo conceptual y diagramas técnicos; la Fase 5 se mantiene como partida reducida porque incluye únicamente las ampliaciones finalmente abordadas o evaluadas dentro del alcance disponible.
